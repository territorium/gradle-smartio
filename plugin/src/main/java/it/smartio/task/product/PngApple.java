...
/**
 * The {@link PngApple} class.
 */
public class PngApple implements Renderer {
  ...
  /**
   * Constructs an instance of {@link PngApple}.
   *
   * @param icon
   * @param logo
   */
  public PngApple(Image icon, Image logo) {
    this.icon = icon;
    this.logo = logo;
  }
  ...
  public void render(File workingDir) throws IOException {
    ...
    for (LaunchImage launcher : LaunchImage.values()) {
      ...
      Image content = PNGTools.createLauncher(this.logo, launcher.width, launcher.height);
      try (OutputStream output = new FileOutputStream(png)) {
        output.write(PNGTools.scalePNG(content, launcher.width, launcher.height));
      }
    }
  ...
  private void createIconIOS(File folder) throws IOException {
    ...
    for (IconSet e : IconSet.values()) {
      ...
      try (OutputStream output = new FileOutputStream(png)) {
        output.write(PNGTools.scalePNG(this.icon, e.size * e.scale, e.size * e.scale));
      }

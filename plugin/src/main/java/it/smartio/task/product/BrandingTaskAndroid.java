...
/**
 * The {@link BrandingTaskAndroid} is used to create all Android & iOS branding details.
 */
public class BrandingTaskAndroid extends BrandingTaskPlatform {
  ...
  protected void render(File logo, File icon, String background, String bgColor, TaskContext context)
    ...
    if (logo.getName().endsWith(".png")) {
      ...
      BufferedImage pngIconRound = ImageIO.read(icon);
      // SVGDocument svgIconRound = SVGTools.createRounded(svgIcon, bgColor);
      // if (background != null) {
      // svgIcon = SVGTools.createSquare(svgIcon, bgColor);
      // }

      renderer = new PngAndroid(pngIcon, pngLogo, pngIconRound);
    } else {
      ...
      renderer = new SvgAndroid(svgIcon, svgLogo, svgIconRound);
    }

    File target = context.getWorkingDir();
    renderer.render(new File(target, "android/res"));
  }
}
